...
### 2.1. Exponential generating functions and effects of sequence transformations
...
### 2.1. Bell numbers, Stirling numbers, Touchard polynomials and operator calculus
...
A final useful relation is the **Dobiński** formula that let us express $T_n(x)$ and the Bell numbers $B_n = T_n(1)$ as an infinite sum:
...
### 3.1. Exponential generating functions
...
The derivative of $I_0$ and $K_0$ are thankfully straightforward ([10.29.3](https://dlmf.nist.gov/10.29.E3)) @NIST:DLMF:
...
These expressions can be simplified due to the following Bessel identity concerning the Wronskian of the modified Bessel functions (see [10.28.2](https://dlmf.nist.gov/10.28.E2)):
...
### 3.2. Series expansion of the e.g.f.

The relatively complicated expression for the e.g.f. of 2-Bell numbers made me question how it can result in integer coefficients. Let us explore the series expansion for the derved e.g.f.s. Consider first the case of sequence A351143:
