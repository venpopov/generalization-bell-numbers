...
\thanks{Computational results and other supporting information is available on \href{https://github.com/venpopov/generalization-bell-numbers}{GitHub}}
\date{}
...
\begin{landscape}
\begin{figure}[p]  % 'p' places it on a page of its own
\centering
...
every node/.style={draw,circle,minimum size=0.25cm,font=\scriptsize}]
\node {0}
    child {node {1}
        child {node {2}
            child {node {2}
